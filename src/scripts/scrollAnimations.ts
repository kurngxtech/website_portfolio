...
function initSectionReveals(reducedMotion: boolean) {
   ...
   if (reducedMotion) {
      const targets = document.querySelectorAll('.reveal-on-scroll, .line-reveal, .contact-card');
      targets.forEach((el) => {
         if (el instanceof HTMLElement) {
            el.style.opacity = '1';
            el.style.transform = 'none';
   ...
   // 2e. CONTACT SECTION (Stagger Contact Channel Cards)
   ...
   if (contactSection) {
      ...
      if (heading) handledElements.add(heading);
      cards.forEach((card) => handledElements.add(card));
         ...
         () => {
            ...
            if (cards.length) {
               animate(
                  cards,
                  { opacity: [0, 1], y: [20, 0] },
                  { delay: stagger(0.08, { start: 0.1 }), duration: 0.45, ease: EASE_EDITORIAL }
               );
            }
         },
         { amount: 0.15 }
